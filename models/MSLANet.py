...
class MSLANet(nn.Module):
    def __init__(self):
        ...
        self.lanet_model = LANet(INPUT_SIZE, NUM_CLASSES)
        self.thresholds = [120]
    ...
    def forward(self):
        ...
        for thresh in self.thresholds:
            ...
            for _ in range(5):
                ...
                lanet_output = self.lanet_model(cropped_img)
                lanet_output = nn.Dropout(p=0.5)(lanet_output)
                lanet_output = lanet_output.view(batch_size, -1)
                lanet_output = nn.Linear(
                    lanet_output.shape[-1], 7)(lanet_output)
