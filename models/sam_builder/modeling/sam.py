...
class Sam(nn.Module):
    ...
    def individual_forward(
        ...
        input_images = torch.stack(
            [self.lbk_preprocess(x["image"]) for x in batched_input], dim=0)
        image_embeddings = self.image_encoder(input_images)
        ...
        for image_record, curr_embedding in zip(batched_input, image_embeddings):
            ...
            refined_masks = self.postprocess_small_regions(
                low_res_masks, iou_predictions, *input_images.shape[2:], is_low_resol)
            if not is_low_resol:
    ...
    def postprocess_small_regions(self, masks, iou_predictions, orig_h, orig_w, is_low_resol):
        ...
        data['masks'] = uncrop_masks(
            data["masks"], [0, 0, orig_w, orig_h], orig_h, orig_w)
