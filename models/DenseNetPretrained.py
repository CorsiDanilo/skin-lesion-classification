...
class DenseNetPretrained(nn.Module):
    def __init__(self, hidden_layers, num_classes):
        ...
        if len(hidden_layers) == 0:
            ...
            self.layers.append(nn.Linear(self.model.classifier.in_features, num_classes, bias=False))
        else:
            self.layers.append(self.dropout)
            for i in range(len(hidden_layers)):
                if i == 0:
                    self.layers.append(nn.Linear(self.model.classifier.in_features, hidden_layers[i], bias=False))
