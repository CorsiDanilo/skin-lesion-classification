...
def test(test_model, test_loader, device, data_name):
    ...
    with torch.no_grad():
        for _, (test_images, test_labels) in enumerate(test_loader):
            ...
            test_epoch_loss_multiclass = loss_function_multiclass(test_outputs, test_labels)
            test_epoch_loss = test_epoch_loss_multiclass
            
            if USE_DOUBLE_LOSS:
                test_labels_binary = torch.zeros_like(test_labels, dtype=torch.long).to(device)
                test_labels_binary[(test_labels == 0) | (test_labels == 1) | (test_labels == 6)] = 1  # Set ground-truth to 1 for classes 0, 1, and 6 (the malignant classes)

                #Second loss: Binary loss considering only benign/malignant classes
                test_outputs_binary = torch.zeros_like(test_outputs[:, :2]).to(device)
                test_outputs_binary[:, 1] = torch.sum(test_outputs[:, [0, 1, 6]], dim=1)
                test_outputs_binary[:, 0] = 1 - test_outputs_binary[:, 1]

                test_epoch_loss_binary = loss_function_binary(test_outputs_binary, test_labels_binary)
        ...
        test_accuracy = accuracy_score(epoch_test_labels.cpu().numpy(), epoch_test_preds.cpu().numpy()) * 100
        test_recall = recall_score(epoch_test_labels.cpu().numpy(), epoch_test_preds.cpu().numpy(), average='macro', zero_division=0) * 100

        print('Test -> Loss: {:.4f}, Accuracy: {:.4f}%, Recall: {:.4f}%'.format(test_loss, test_accuracy, test_recall))
...
def get_model(model_path, device):
    ...
    if os.path.exists(conf_path):
        print("--Model-- Old configurations found. Using those configurations for the test.")
    ...
    if type == "resnet24":
        ...
        model = InceptionV3Pretrained(NUM_CLASSES if configurations is None else configurations["num_classes"]).to(device)
        normalization_stats = IMAGENET_STATISTICS
    elif type == "standard":
        model = ViT_standard(in_channels=INPUT_SIZE if configurations is None else configurations["input_size"],
                             patch_size=PATCH_SIZE if configurations is None else configurations["patch_size"],
                             d_model=EMB_SIZE if configurations is None else configurations["emb_size"],
                             img_size=IMAGE_SIZE if configurations is None else configurations["image_size"],
                             n_classes=NUM_CLASSES if configurations is None else configurations["num_classes"],
        ...
        model = ViT_pretrained(NUM_CLASSES if configurations is None else configurations["num_classes"], pretrained=True).to(device)
        normalization_stats = IMAGENET_STATISTICS
    elif type == "efficient":
...
if __name__ == "__main__":
    model_path = "resnet24_2023-12-10_12-29-49" # Name of the sub-folder into "results" folder in which to find the model to test (e.g. "resnet24_2023-12-10_12-29-49")
