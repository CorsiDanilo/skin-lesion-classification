...
def main():
    ...
    # CHECKPOINT_GEN = os.path.join("checkpoints", "GAN_GEN_6_45.pth")
    ...
    depth = 7
    epoch = 25
    dis_checkpoint = os.path.join(
        "checkpoints", f"GAN_DIS_{depth}_{epoch}.pth")
    gen_checkpoint = os.path.join(
        "checkpoints", f"GAN_GEN_{depth}_{epoch}.pth")
    gen_shadow_checkpoint = os.path.join(
        "checkpoints", f"GAN_GEN_SHADOW_{depth}_{epoch}.pth")

    style_gan.dis.load_state_dict(torch.load(
        dis_checkpoint, map_location=device))
    style_gan.gen.load_state_dict(torch.load(
        gen_checkpoint, map_location=device))
    style_gan.gen_shadow.load_state_dict(torch.load(
        gen_shadow_checkpoint, map_location=device))
