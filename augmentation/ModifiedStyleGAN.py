...
class Generator(nn.Module):
    ...
    def forward(self,
        ...
        if not self.conditional:
            ...
            print(f"Latents_in shape is {latents_in.shape}")
...
class Resnet50Styles(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        self.linear64 = nn.Linear(256, 256)
        self.linear16 = nn.Linear(1024, 256)
        self.linear8 = nn.Linear(2048, 256)
...
class StyleGAN:
    ...
    def __init__(self,
        ...
        print(f"Generator has {len(list(self.gen.parameters()))} parameters")
        print(
            f"Distriminator has {len(list(self.dis.parameters()))} parameters")

        # TODO: review this
        # Total = 101
        # gen_freeze_point = 95
        # for i, param in enumerate(self.gen.parameters()):
        #     if i < gen_freeze_point:
        #         param.requires_grad = False
        # # Total = 51
        # dis_freeze_point = 30
        ...
        for current_depth in tqdm(range(start_depth, self.depth), desc="Depth loop"):
            ...
            for epoch in tqdm(range(1, epochs[current_depth] + 1), desc="Epoch loop at depth %d" % (current_depth + 1)):
                ...
                for i, batch in enumerate(tqdm(data, desc="training"), 1):
                    ...
                    self.random_style = torch.randn(1, 4, 256).to(self.device)

                    assert self.resnet_style_1.shape == (
                        1, 7, 256), f"Resnet style 1 is not of shape (7, 256), but {self.resnet_style_1.shape}"
                    assert self.resnet_style_2.shape == (
                        1, 7, 256), f"Resnet style 2 is not of shape (7, 256), but {self.resnet_style_2.shape}"
