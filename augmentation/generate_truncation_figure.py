...
def draw_truncation_trick_figure(png, gen, out_depth, seeds, psis):
    ...
    with torch.no_grad():
        latents_np = np.stack([np.random.RandomState(
            seed).randn(latent_size) for seed in seeds])
        latents = torch.from_numpy(latents_np.astype(np.float32))
        # [seed, layer, component]
        dlatents = gen.g_mapping(latents).detach().numpy()
        ...
        for row, dlatent in enumerate(list(dlatents)):
            row_dlatents = (dlatent[np.newaxis] - dlatent_avg) * \
                np.reshape(psis, [-1, 1, 1]) + dlatent_avg
