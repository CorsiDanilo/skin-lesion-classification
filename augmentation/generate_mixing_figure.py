...
def draw_style_mixing_figure(png, gen, out_depth, src_seeds, dst_seeds, style_ranges):
    ...
    with torch.no_grad():
        ...
        src_latents_np = np.stack([np.random.RandomState(
            seed).randn(latent_size, ) for seed in src_seeds])
        dst_latents_np = np.stack([np.random.RandomState(
            seed).randn(latent_size, ) for seed in dst_seeds])
